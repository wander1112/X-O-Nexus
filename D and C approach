...
def cpu_move_dc(b_index):
    ...
    if big_boardgraph.vertices[b_index - 1].val == "":
        ...
        allowed_boards = [i for i in range(9) if big_boardgraph.vertices[i].val == ""]
...
w = tk.Tk()
w.geometry("1024x720")
w.title("X-O Nexus  [Divide & Conquer Mode]")
w.resizable(False, False)
f_bg = tk.Frame(w, bd=2, relief="ridge", bg="#547792")
f_bg.place(x=0, y=0, height=720, width=1024)
tk.Label(w, text="X-O Nexus  [D&C]", font=("Arial", 24, "bold"),
         bg="#547792", fg="#E1EBEE").place(x=400, y=40)
sf = []
buttons = []
