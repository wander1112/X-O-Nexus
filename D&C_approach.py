...
def createList(N): # The function has a time complexity of O(N^2).
    g = Graph()
    for _ in range(N * N):
        g.addVertex("")
    winning_lines = []
...
def dac_check_line(boardgraph, line, symbol):# The function has a time complexity of O(N).
...
def sb_checkwinner(f_index):# The function has a time complexity of O(1).
...
def big_board_check_winner():# The function has a time complexity of O(N).
    for symbol in ["X", "O"]:
        for line in WINNING_LINES:
...
def displaymove(a, v, f):
    """
    a = cell index (0-based) the CPU played — also the target frame index
    v = "allowed" or "notallowed"
    f = frame index (0-based) the CPU played in
    """
    cpu_frame = f + 1
    cpu_cell  = a + 1
    if v == "notallowed":
        move_label.config(
            text=f"CPU played F{cpu_frame} C{cpu_cell}. Your next move should be in the highlighted yellow frame(s)."
        )
    else:
        move_label.config(
            text=f"CPU played F{cpu_frame} C{cpu_cell}. Your next move should be in the highlighted yellow frame."
        )
...
def undo_move(f, c, original_big_board_val):# The function has a time complexity of O(1).
...
def restore_state(big_snap, small_snap):# The function has a time complexity of O(81).
...
def cpu_turn(b_index):
    ...
    forced_closed = target_frame_closed or target_has_no_moves
    if forced_closed:
